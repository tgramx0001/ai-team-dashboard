...
class TestTerminalDeepSecurity(unittest.TestCase):
    ...
    def test_7_output_limits_enforced_on_both_stdout_and_stderr(self):
        """7. Output limits apply to both stdout and stderr independently."""
        with patch.dict(os.environ, {"TERMINAL_MAX_OUTPUT": "5000"}):
            cmd = (
                "python3 -c \"import sys; sys.stdout.buffer.write(b'O' * 8000); sys.stdout.flush(); sys.stderr.buffer.write(b'E' * 8000); sys.stderr.flush()\""
            )
            res = client.post("/api/workspace/terminal", json={
                "command": cmd,
                "path": TEST_WORKSPACE_DIR
            ...
            self.assertIn("Output truncated: exceeded 5000 chars", data["stdout"])
            self.assertIn("Error output truncated: exceeded 5000 chars", data["stderr"])
            self.assertLessEqual(len(data["stdout"]), 5200)
            self.assertLessEqual(len(data["stderr"]), 5200)
